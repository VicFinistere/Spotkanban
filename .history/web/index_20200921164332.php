...
function create_task($task_name, $task_description, $task_status, $task_team){
  ...
  if($task_status != ""){
    ...
    $req = sprintf($format, mysql_real_escape_string($task_name), mysql_real_escape_string($task_status), mysql_real_escape_string($task_description), mysql_real_escape_string($task_team));
  } else {
    $format = "INSERT INTO task (name, description, team) VALUES ('%s', '%s', '%s') ";
    $req = sprintf($format, mysql_real_escape_string($task_name), mysql_real_escape_string($task_description), mysql_real_escape_string($task_team));
  }
  $conn->set_charset("utf8");
